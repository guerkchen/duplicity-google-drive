...
def get_file_entry(abs_path, src_dir):
    # get additional informations for every file
    size = os.path.getsize(abs_path) # size
    ctime = os.path.getctime(abs_path) # last modified date

    # the file is referenced by it's relative path, starting at the src_dir. This way we can easily move the src_dir to another location
    rel_path = os.path.relpath(abs_path, src_dir)

    enc_filename = 'enc-' + sha256(rel_path.encode('utf-8')).hexdigest() + '.gpg' # create enc filename (by hashing the relative path)
    file_entry =  File_Entry(size, ctime, rel_path, enc_filename, 0)
...
def write_masterfile(masterfile_abs_path, masterfile_abs_path_old, folder_struc, password_file):
...
def main(argv):
    ...
    global src_dir, backup_dir, password_file, config_file, delete, restore
    ...
    if os.path.isfile(config_file):
        ...
        src_dir = config['VARIABLE']['SRC_FOLDER']
        backup_dir = config['VARIABLE']['BACKUP_FOLDER']
    ...
    if src_dir == "" or backup_dir == "" or password_file == "":
    ...
    masterfile_path = os.path.join(backup_dir, MASTERFILE_NAME)
    ...
    for is_entry in is_struct:
        ...
        if is_entry.rel_path not in should_dict:
            ...
            is_entry.enc_size = encrypt_and_backup(is_entry.rel_path, backup_dir, is_entry.enc_filename, password_file)
        ...
        if is_entry.enc_filename != should_entry.enc_filename:
            ...
            if should_entry.enc_filename in lost_found_backup_dict:
                if should_entry.enc_size == lost_found_backup_dict[should_entry.enc_filename]: 
                    ...
                    logging.debug("backup file for '" + is_entry.rel_path + "' (" + str(is_entry.size) + ") must be renamed")
                    corrupt_files_counter += 1
                    command.run(['mv', os.path.join(backup_dir, should_entry.enc_filename), os.path.join(backup_dir, is_entry.enc_filename)])
        ...
        if is_entry.ctime != should_entry.ctime or is_entry.size != should_entry.size or is_entry.enc_size != should_entry.enc_size:
            ...
            should_entry.enc_size = encrypt_and_backup(is_entry.rel_path, backup_dir, should_entry.enc_filename, password_file)
    ...
    for should_entry in should_dict.values():
        ...
        if restore:
            ...
            decrypt_and_restore(should_entry.filename, backup_dir, should_entry.enc_filename, password_file)
